...
interface RoutineEditContentProps {
  initialTitle: string;
  initialUrl?: string;
...
export default function RoutineEditContent({
  initialTitle,
  initialUrl,
  initialPeriod,
  ...
  onDelete,
  onClose,
}: RoutineEditContentProps) {
  const [title, setTitle] = useState(initialTitle);
  const [url, setUrl] = useState(initialUrl ?? '');
  const [period, setPeriod] = useState<RoutinePeriod>(initialPeriod);
  const [pomodoro_count, setPomodoroCount] = useState(initialPomodoroCount);
  const [category, setCategory] = useState<RoutineCategory>(initialCategory);

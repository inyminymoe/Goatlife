...
import { FileAttach } from '../post/FileAttach';

const AVAILABLE_TOPICS = ['공지', '정보', '질문', '잡담', '모집'];
...
export default function BoardEditForm({
  ...
  const { mutate: submitUpdate, isPending } = useMutation({
    ...
    onError: error => {
      console.error('[BoardEditForm] update failed', error);
  ...
  return (
    <main className="flex-1 px-6 md:px-6 py-6 bg-grey-100 col-span-2 space-y-4">
      ...
      <div className="flex items-center justify-between">
        <h2 className="brand-h2 font-brand text-grey-900">글 수정</h2>
        <Button
          type="button"
          variant="primary"
          size="md"
